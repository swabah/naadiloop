...
- **Repository process files out of scope.** `issues/README.md`, `plans/README.md`, and `done/README.md` currently reference a third product (TradeRoute). Out of scope for a single-issue refinement; flag for a separate housekeeping pass if it matters for the demo. *Addressed in the ISSUE-002–011 backlog-planning pass: `workflow.md`, `issues/README.md`, and `plans/README.md` were rewritten for Naadi Loop; `done/README.md` was confirmed already clean.*
...
- Implemented the issue-plan enum values as the frozen contract and used minimal typed JSONB payloads: medication schedule/duration, test name, referral specialty, and follow-up reason.
- Kept every tRPC procedure, including `auth.login`, as a typed `NOT_IMPLEMENTED` stub. The demo role toggle is intentionally local and supplies an `x-demo-role` context header.
- Registered the ten routes in code and wrapped them in a responsive, reference-informed Naadi Loop shell; route content remains placeholder-only.
- Linked Neon project `old-cloud-29987760`, created isolated branch `dev-issue-001`, and pulled pooled/direct URLs into the ignored local `.env`. The active session did not hot-load Neon MCP tools after initialization, so authenticated Neon CLI commands performed branch setup and Drizzle performed schema/data verification.
- Verified `pnpm check`, `pnpm typecheck`, `pnpm build`, both dev servers, all ten browser routes, expected tRPC `NOT_IMPLEMENTED` behavior, mobile/desktop screenshots, Drizzle push, idempotent seed, and the seeded row/action counts.
